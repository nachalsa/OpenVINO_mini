...
# [추가] 근접 감지 트리거 기능을 위한 새로운 예제 함수
def example_webcam_proximity_trigger():
    """
    웹캠 실시간 포즈 추정 및 근접 감지 트리거 예제
    사람이 카메라에 가까이 다가가면 설정된 명령어가 실행됩니다.
    """
    ...
    # 근접 감지 기능이 활성화된 포즈 추정기 초기화
    estimator = PoseEstimator(
        enable_proximity_trigger=True,    # 기능 활성화
        proximity_threshold=0.5,         # 프레임 면적 대비 15% 이상 차지할 때 트리거 (환경에 맞게 조정)
        trigger_command="xdotool set_desktop 1" # Linux에서 화면 잠금. 다른 OS는 명령어 수정 필요
    )
    
    try:
        # 팝업 창을 사용하여 시각적 피드백(바운딩 박스, 상태 텍스트) 확인
        estimator.estimate_pose_webcam(camera_id=0, use_popup=True)
    except Exception as e:
        print(f"오류 발생: {e}")
...
if __name__ == "__main__":
    ...
    # [수정] 새로운 예제 함수 호출을 위한 elif 블록 추가
        ...
        example_webcam_proximity_trigger()
    else:
        print("잘못된 선택입니다.")
